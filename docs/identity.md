...
```
polkadot-js-api --ws wss://testnet.liberland.org query.identityPallet.identityOf 5HGZfBpqUUqGY7uRCYA6aRwnRHJVhrikn8to31GcfNcifkym
```
If you see results, run the `clearIdentity` command below

### Clear Identity
```bash
polkadot-js-api --ws wss://testnet.liberland.org tx.identityPallet.clearIdentity --seed "bargain album current caught tragic slab identify squirrel embark black drip imitate"
```
...
```bash
polkadot-js-api --ws wss://testnet.liberland.org tx.identityPallet.setIdentity '{
    "display": {
        "Raw": "Steve Harvey"
    },
    "web": {
...
```bash
polkadot-js-api --ws wss://testnet.liberland.org query.identityPallet.registrars 
```
```json
{
  "registrars": [
...
```bash
polkadot-js-api --ws wss://testnet.liberland.org tx.identityPallet.requestJudgement 1 500 --seed "bargain album current caught tragic slab identify squirrel embark black drip imitate"
```
...
```bash
polkadot-js-api --ws wss://testnet.liberland.org tx.identityPallet.provideJudgement 1 5HGZfBpqUUqGY7uRCYA6aRwnRHJVhrikn8to31GcfNcifkym KnownGood --seed "exercise museum credit crystal various nature defy human cable any split help"
```
